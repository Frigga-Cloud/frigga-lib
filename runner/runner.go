...
type Runner struct {
	...
	onLogCallback      func([]byte) // User-defined function to be called when a new output line is generated by the system call and is injected into this function
...
func NewRunner() *Runner {
	// instantiate a new runner object and
	r := &Runner{}
	r.id = calculateMD5([]byte(fmt.Sprintf("%v", time.Now().UnixNano()))) // md5 (current_epoch + name)
	r.status = "PENDING"
	return r
}
...
func (r *Runner) SetTimeout(timeout int, callbacks ...func([]byte)) {
	r.timeout = timeout
	if len(callbacks) == 1 {
		r.onTimeoutCallback = callbacks[0]
	}
}
...
func (r *Runner) SetVerificationPhrase(phrase string, callbacks ...func([]byte)) {
	r.verificationPhrase = phrase

	if len(callbacks) == 1 {
		r.onSuccessCallback = callbacks[0]
	}
}
...
func (r *Runner) OnLogCalback(callback func([]byte)) {
	r.onLogCallback = callback
}
...
func (r *Runner) OnSuccessCallback(callback func([]byte)) {
...
func (r *Runner) OnCompleteCallback(callback func([]byte)) {
...
func (r *Runner) OnFailCallback(callback func([]byte)) {
...
func (r *Runner) OnTimeoutCallback(callback func([]byte)) {
...
func (r *Runner) Execute(command string, callbacks ...func([]byte)) ([]byte, error) {
	// if multiple strings arguments are provided, then concat that with &&. e.g. <cmd1> && <cmd2> && ...<cmd-n>

	if len(callbacks) == 1 {
		r.onFailCallback = callbacks[0]
	} else if len(callbacks) == 2 {
		r.onFailCallback = callbacks[0]     // on fail callback
		r.onCompleteCallback = callbacks[1] // on success callback
	}
	...
	go func() {
		...
		for scanner.Scan() {
			...
			if r.onLogCallback != nil {
				r.onLogCallback(scanner.Bytes()) // Invoke the user-defined success callback
			}
		}
	...
	if r.status == "TIMEDOUT" {
		...
		if r.onTimeoutCallback != nil {
			r.onLogCallback(r.logBuffer) // Invoke the user-defined success callback
		}
		return r.logBuffer, nil
	}
	...
	if r.onFailCallback != nil {
		r.onFailCallback(r.logBuffer) // callback
	}
	return r.logBuffer, nil
}
